<template>
  <div id="firecontrolEquipment"
      ...
      <div class="table">
        <el-table :data="tableData"
          ...
          <el-table-column label="操作"
                           v-if="hasAuthority('03040104')||hasAuthority('03040105')"
                           align="center">
    ...
    <el-dialog title="新增设备"
      ...
      <addEquipment :equirementTypes="getEquirementTypes4List"
                    ref="addEquipment"
                    equirementTypeName="弱电系统"
...
export default {
  ...
  methods: {
    ...
    async downLoadModel () { // 下载设备模板
      axios({ method: 'GET', url: '/cms/api/equriment-excel/file?typeId=4', responseType: 'blob' }).then(res => {
    ...
    async downLoadModel1 (index, row) { // 下载错误文件
      axios({ method: 'GET', url: `/cms/api/equriment-excel/upload?excelId=${row.id}&typeId=4`, responseType: 'blob' }).then(res => {
        let url = window.URL.createObjectURL(new Blob([res.data]))
        let link = document.createElement('a')
        link.style.display = 'none'
        link.href = url
