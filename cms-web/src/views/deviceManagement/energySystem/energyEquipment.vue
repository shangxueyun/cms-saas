  ...
  <div id="energyEquipment"
    ...
    <div class="search-bar">
      <div class="searbar-left">
        ...
        <span v-if="energyType==='METER'">
          <el-select v-model="watchType"
                     @change="watchTypeChange"
                     placeholder="表类型"
                     clearable>
            <el-option label="普通表"
        ...
        <span v-if="watchType==='ORDINARY_TABLE'">
      ...
      <div class="table">
        <el-table :data="tableData"
          ...
          <el-table-column prop="name"
                           label="设备名称"
                           align="center"></el-table-column>
          <el-table-column :formatter="formatenergyType"
                           label="能耗类型"
                           align="center"></el-table-column>
          <el-table-column :formatter="formatinstrumentType"
                           label="仪表类型"
                           align="center"></el-table-column>
    ...
    <el-dialog title="新增设备"
      ...
      <addEquipment :equirementTypes="getEquirementTypes25List"
                    ref="addEquipment"
                    equirementTypeName="能耗系统"
        ...
        <el-table-column property="address"
          ...
          <template slot-scope="scope">
            <span class="export-error"
                  v-if="errorType==='equipment'"
                  @click="downLoadModel1(scope.$index, scope.row)">导出错误提示</span>
            <span class="export-error"
                  v-if="errorType==='energy'"
                  @click="downLoadModel2(scope.$index, scope.row)">导出错误提示</span>
          </template>
        </el-table-column>
      </el-table>
      <span slot="footer"
...
export default {
  ...
  data () {
    return {
      ...
      errorType: 'equipment', // equipment设备 能耗energy（导入错误类型）
      errorPrompt: false,
  ...
  methods: {
    chartTypeChange (id) {
      if (id !== 27 && this.energyType === 'LAMP' || this.energyType === 'VRV') {
        this.energyType = 'METER'
      }
      this.currentSel()
    },
    ...
    async downLoadModel () { // 下载设备模板
      axios({ method: 'GET', url: '/cms/api/equriment-excel/file?typeId=25', responseType: 'blob' }).then(res => {
    ...
    async downLoadModel1 (index, row) { // 下载错误文件
      axios({ method: 'GET', url: `/cms/api/equriment-excel/upload?excelId=${row.id}&typeId=25`, responseType: 'blob' }).then(res => {
        let url = window.URL.createObjectURL(new Blob([res.data]))
        let link = document.createElement('a')
        link.style.display = 'none'
        link.href = url
    ...
    // -----------------------------------------------------
    async downLoadModel2 (index, row) {
      let url = ''
      url = `/cms/api/energy_excel/file?projectManagementId=${this.projectManagementId}&equirementTypeChildId=${this.chartType}&energyType=${this.energyType}&useStatus=${this.useStatus}&tenantName=${this.tenName}`
      if (row) {
    ...
    importModel2 (event) {
      ...
      if (index > 0 && type !== 'xlsx' && type !== 'xls') {
        ...
        context.http.post('/cms/api/energy_excel/file', form, { headers: { 'Content-Type': 'multipart/form-data' } }).then(res => {
          if (res.status === 200) {
            this.errorType = 'energy'
            this.queryExportResult2(res.data.id)
            event.target.value = ''
          }
        }).catch(error => {
          event.target.value = ''
    ...
    formatenergyType (row, column) {
      if (row.energyType === 'METER') {
        return '仪表'
      } else if (row.energyType === 'SWITCH') {
        return '开关'
      } else if (row.energyType === 'LAMP') {
        return '照明'
      } else if (row.energyType === 'VRV') {
        return '空调'
      } else if (row.energyType === 'OTHER') {
        return '其他'
      }
    },
