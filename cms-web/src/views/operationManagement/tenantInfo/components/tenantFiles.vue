  ...
  <div class="tenant-files">
    <el-form :model="tenantDetail"
      ...
      <div class="tf-left float-left">
        ...
        <div style="width:100%;border-bottom: 1px dotted #eee; margin: 0 15px 15px;"></div>
        <el-form-item>
          <div class="cttitle">经营类型</div>
          <span v-if="readOnly">{{formatTenantUse(tenantDetail.tenantUse)}}</span>
          <el-select v-model="tenantDetail.tenantUse"
                     @change="tenantUseChange"
                     size="small"
                     v-else
                     placeholder="请选择">
            <el-option label="办公"
                       value="WORK"></el-option>
            <el-option label="农贸"
                       value="MARKET"></el-option>
            <el-option label="商铺"
                       value="SHOP"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item prop="referred"
                      v-if="tenantDetail.tenantUse==='SHOP'||tenantDetail.tenantUse==='MARKET'"
      ...
      <div class="tf-right float-left"
        ...
        <div style="color:#777;text-align: left;">店铺logo</div>
...
export default {
  ...
  methods: {
    formatTenantUse (tenantUse) {
      if (tenantUse === 'WORK') {
        return '办公'
      } else if (tenantUse === 'MARKET') {
        return '农贸'
      } else if (tenantUse === 'SHOP') {
        return '商铺'
      }
    },
    handlerEdit1 () { // 编辑
      this.readOnly = false
      this.tenantUseChange(this.tenantDetail.tenantUse)
    ...
    handleIndustryChange (e) {
      let id = e[e.length - 1]
      this.tenantDetail.standardIndustryId = id ? id + '' : null
      let name
    ...
    tenantUseChange (val) {
      if (val === 'WORK') {
        this.tenantDetail.tenantUse = 'WORK'
    ...
    confirm () {
      this.$refs['tenantDetail'].validate((valid) => {
        if (valid) {
          ...
          this.tenantDetail.standardIndustryId = this.tenantDetail.standardIndustryId ? this.tenantDetail.standardIndustryId : null
