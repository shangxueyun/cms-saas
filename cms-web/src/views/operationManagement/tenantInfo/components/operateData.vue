...
export default {
  ...
  methods: {
    ...
    drawLineEnergy (dateArray, dataArray1, dataArray2) {
      let _this = this
      const max = Math.max.apply(null, dataArray1, dataArray2)
      var yMax = max
      ...
      let option = {
        ...
        tooltip: {
          ...
          formatter: function (params) {
            const item = params[0]
            return item.axisValue + '<br>销量:' + _this.formatData(params[0].data) + '吨' + '<br> 营业额：' + _this.formatData(params[1].data) + '万元'
          }
        },
        grid: {
          left: '3%',
    ...
    getBusinessData () {
      ...
      context.http.get('/cms/api/tenantBusiness/chartV2', {
        tenantId: this.tenantDetail.id,
        startTime: this.startMonth + '-01',
        endTime: this.endMonth + '-01'
        ...
        if (res.status === 200 && res.data.length > 0) {
          ...
          res.data.forEach(ele => {
            let time = moment(ele.xTime).format('YYYY-MM')
            dateArray.push(time)
            dataArray1.push(ele.totalAssets)
            dataArray2.push(ele.businessIncome)
          })
          this.drawLineEnergy(dateArray, dataArray1, dataArray2)
        } else {
