      ...
      <div class="clearfix"
        ...
        <div class="plc-search clearfix"
          ...
          <div class="btnDiv"
            ...
            <el-button class="tban tban5"
                       style="position: relative;"><i class="iconfont icon-cms_daorushuju-"></i> 导入税收累计数据
              <input type="file"
                     style="position: absolute;opacity: 0;left: 0;width: 110px;height: 35px;font-size: 0;top: -3px;cursor: pointer;"
  ...
  methods: {
    ...
    importModel2 (event) {
      ...
      if (index > 0 && type !== 'xlsx' && type !== 'xls') {
        ...
        context.http.post('/cms/api/tenantTax-excel/file/total', form, { headers: { 'Content-Type': 'multipart/form-data' } }).then(res => {
          if (res.status === 200) {
            this.$emit('queryExportResult', res.data.id, false) // true代表经营数据 false税收数据
            // this.queryExportResult(res.data.id)
            event.target.value = ''
