      ...
      <div class="search-bar">
        <div class="operator">
          ...
          <el-input v-model="seachParams.queryShopNumber"
                    v-if="seachParams.onRent"
                    style="width:180px"
                    placeholder="房源号"></el-input>
          <el-input v-model="seachParams.sortBuildingArea"
                    v-else
                    type="number"
                    style="width:180px"
                    placeholder="面积"></el-input>
        ...
        <div class="table">
          <el-table :data="tableData"
            ...
            <el-table-column prop="buildingArea"
                             label="面积/㎡"
                             align="center"></el-table-column>
            <el-table-column label="楼宇类型"
                             align="center">
              <template slot-scope="scope">
                <div>
                  <div>{{buildingBusinessFormat(scope.row.building.buildingBusiness)}}</div>
                </div>
              </template>
            </el-table-column>
            <el-table-column label="使用状态"
              ...
              <template slot-scope="scope">
                <div>
                  <div>{{useStatusFormat(scope.row.building.buildingBusiness,scope.row.onRent,scope.row.onRentStatistics)}}</div>
    ...
    <el-dialog class="addHourse housingList_addShop"
      ...
      <div>
        ...
        <el-form class="form"
          ...
          <div class="content">
            ...
            <el-form-item prop="projectManagementId">
              ...
              <el-select v-model="addData.projectManagementId"
                         size="small"
                         placeholder="请选择"
                         @change="chooseItem('project', addData.projectManagementId)">
            ...
            <el-form-item prop="buildingId">
              ...
              <el-select v-model="addData.buildingId"
                         size="small"
                         placeholder="请选择"
                         @change="chooseItem('hourse', addData.buildingId)">
          ...
          <div class="content">
            <el-form-item style="width:60%!important">
              <span>企业<i>*</i></span>
            ...
            <el-form-item prop="moveIntoType">
              <span>类别</span>
              <el-select size="small"
                         ref="select"
                         style="width:100%!important"
                         v-model="addData.moveIntoType"
                         placeholder="请选择">
                <el-option label="一手"
                           value="一手"></el-option>
                <el-option label="二手"
            ...
            <el-form-item style="width:60%!important"
              ...
              <div>
                <span>起始日期</span>
                <el-date-picker v-model="addData.startTime"
                                size="small"
                                type="date"
                                style="width:100%"
              ...
              <div>
                <span>截止日期</span>
                <el-date-picker v-model="addData.endTime"
                                size="small"
                                style="width:100%"
                                type="date"
                                value-format="yyyy-MM-dd"
            ...
            <el-form-item>
              ...
              <el-input placeholder="请输入内容"
                ...
                <el-select v-model="addData.rentPriceType"
                           style="width:135px"
                           slot="prepend"
                           placeholder="请选择">
          ...
          <div class="content">
            <div class="roomsDTOS"
                 v-for="(item,index) in addData.roomsDTOS">
              <el-form-item :prop="'roomsDTOS['+index+'].floorId'"
                            :rules="addDataRules.floorId">
                <span>楼层<i>*</i></span>
                <el-select v-model="item.floorId"
                           size="small"
                           placeholder="请选择">
              ...
              <el-form-item :prop="'roomsDTOS['+index+'].shopNumber'"
                            :rules="addDataRules.shopNumber">
                <span>房源号<i>*</i></span>
                <el-input v-model="item.shopNumber"
                          size="small"></el-input>
              </el-form-item>
              <el-form-item :prop="'roomsDTOS['+index+'].buildingArea'"
                            :rules="addDataRules.buildingArea">
                <span>面积/㎡<i>*</i></span>
                <el-input v-model="item.buildingArea"
                          size="small"
                          @input="getHsRate"
                          type="number"></el-input>
              </el-form-item>
              <el-form-item v-if="buildingBusiness==='SELF_USE'">
                <span>加入出租率统计<i>*</i></span>
                <el-select v-model="item.onRentStatistics"
                           size="small"
                           placeholder="请选择">
                  <el-option label="是"
              ...
              <el-form-item prop="productionCertificateNo">
                ...
                <el-input maxlength="50"
                          v-model="item.productionCertificateNo"
                          size="small"></el-input>
              </el-form-item> -->
              <el-form-item style="width:100%"
                ...
                <div style="display: flex;margin-top:10px;">
                  ...
                  <div v-for="(item1, index1) in item.imgUrls"
                       class="imgItem"
                       :key="index1">
                    <img :src="item1">
                    <i @click="deleFile(item1, index,index1)"
                       class="iconfont icon-jinakangbaoicons18"></i>
                  </div>
                  <aliUpload class="aliUpload deviceDetails"
                             v-if="item.imgUrls.length < 5"
                             ...
                             :iconImg="iconImg"
                             :uploadIcon="true"
                             @setAnnex="(annexURL, annexName)=>{setAnnex(annexURL, annexName,index)}" />
                </div>
              </el-form-item>
              <div class="delCoupon"
                   v-if="addData.roomsDTOS.length>1"
      ...
      <div slot="footer"
           class="slotfooter">
        <span @click="addDialogClose">取 消</span>
...
export default {
  ...
  watch: {
    'seachParams.buildingId' (newValue, oldValue) { // 当前楼宇能否提供楼层面积
      if (!newValue) {
        this.provideFloorArea = false
        this.seachParams.onRent = true
        return
      }
      this.provideFloorArea = this.options2.filter(item => item.id === newValue)[0].provideFloorArea
      if (!this.provideFloorArea) {
        this.seachParams.onRent = true
      }
      console.log(newValue, this.provideFloorArea)
    },
    'addEnterInfoId' (newValue, oldValue) {
      this.$set(this.addData, 'rzEnterpriseId', '')
  ...
  data () {
    ...
    return {
      provideFloorArea: true, // 当前楼宇能否提供楼层面积
      buildingBusiness: null, // RENT("出租"), SELL("出售"), SELF_USE("自用")
      addEnterInfoId: '',
      addData: {
        organizeId: '',
  ...
  methods: {
    useStatusFormat (type, onRent, onRentStatistics) {
      let label = ''
      if (type === 'RENT') {
        label = onRent ? '已租' : '空置'
      } else if (type === 'SELL') {
        ...
        label = onRent ? onRentStatistics ? '已租' : '已自用' : '空置'
      }
      return label
    },
    buildingBusinessFormat (type) { // RENT("出租"), SELL("出售"), SELF_USE("自用")
      switch (type) {
        case 'RENT':
          return '出租'
        case 'SELL':
    ...
    addmore () {
      ...
      this.addData.roomsDTOS.push(item)
    },
    delCoupon (index) {
      this.addData.roomsDTOS.splice(index, 1)
    },
    handleSelectTeant (item) {
      console.log(item)
      this.$nextTick(() => {
        this.$set(this.addData, 'rzEnterpriseId', item.id)
      })
      // this.addData.rzEnterpriseId = item.id
      this.$refs.select.focus()
    },
    clearFn () {
      // this.addData.rzEnterpriseId = ''
      this.$set(this.addData, 'rzEnterpriseId', '')
      this.$refs.select.focus() // 为了使autocomplete失去焦点 解决autocomplete删除不能继续输入bug
    },
    ...
    addHoursing () {
      ...
      // this.addData.projectManagementId = this.seachParams.projectManagemantId
      // this.addData.buildingId = this.seachParams.buildingId
      // this.projectList = this.projectOptions
      // this.hourList = this.options2
      // this.buildingList = this.options3
    ...
    chooseItem (state, id) {
      this.addData.roomsDTOS.forEach((item, index) => { // 切换项目楼宇 房源清空
        item.floorId = null
      })
      if (state === 'project') {
        this.selProject = !this.selProject
        this.addData.projectManagementId = id
        this.addData.buildingId = ''
        this.buildingList = []
        context.http.get(`/cms/api/project/${id}/buildings`, { useStatus: 'USING' }).then(res => {
        ...
        this.selHourse = !this.selHourse
        this.addData.buildingId = id
        this.buildingBusiness = this.hourList.filter(item => id === item.id)[0].buildingBusiness // 选中楼宇的楼宇类型为自用 出租或出售
        // console.log(this.buildingBusiness)
        if (this.buildingBusiness !== 'SELF_USE') {
          // this.addData.onRentStatistics = null
        }
        context.http.get('/cms/api/floors/building/' + this.addData.buildingId).then(res => {
    ...
    addHourse (formName) {
      if (!this.addData.rzEnterpriseId || this.addEnterInfoId === '') {
        this.$message.error('请选择企业')
        return
      }
      if (moment(this.addData.endTime).isBefore(this.addData.startTime)) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          context.http.post('/cms/api/rooms/enterpriseRooms', addData).then(res => {
            console.log(res)
    ...
    setAnnex (annexURL, annexName, index) {
      this.addData.roomsDTOS[index].imgUrls.push(annexURL)
    },
    deleFile (item, index, index1) {
      this.addData.roomsDTOS[index].imgUrls.splice(index1, 1)
    },
