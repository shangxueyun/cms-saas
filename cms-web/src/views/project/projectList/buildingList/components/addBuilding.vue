    ...
    <div class="dl-content clearfix">
      <el-form :model="addData"
        ...
        <div class="step1"
          ...
          <div style="margin-bottom:15px;"
            ...
            <el-form-item :label-width="formLabelWidth"
              ...
              <span v-if="addData.id">
                {{formatBuildingBusiness(addData.buildingBusiness)}}
              </span>
              <el-select v-model="addData.buildingBusiness"
                         v-else
                         placeholder="请选择">
          ...
          <div class="dc-right"
            ...
            <el-form-item prop="buildingArea"
                          ...
                          v-if="addData.assetsType === 'BUILDING'"
                          style="margin-bottom: 20px;margin-left: 0">
              <div class="cttitle">可用面积<i class="required">*</i>
              <el-tooltip class="item"
                          effect="dark"
                          placement="right-start">
                <div slot="content">
                  <div>楼宇类型为出租：填写可出租面积</div>
                  <div>楼宇类型为出售：填写可出售面积</div>
                  <div>楼宇类型为自用：填写自用面积</div>
                </div>
            ...
            <el-form-item prop="buildingArea"
                          :label-width="formLabelWidth"
                          v-if="addData.assetsType === 'BUILDING' && addData.buildingBusiness === 'SELF_USE'"
                          style="margin-bottom: 20px;margin-left: 0">
                          <!-- 当楼宇类型选择为“自用”时 显示 -->
              <div class="cttitle">可出租面积 <span>(注：填写面积默认加入出租率统计)</span> </div>
              <el-input v-model="addData.canRentArea"
                        type="number"
                        placeholder="请输入内容">
                <template slot="append">㎡</template>
              </el-input>
            ...
            <el-form-item :label-width="formLabelWidth"
              ...
              <el-select v-model="addData.provideFloorArea"
                         placeholder="请选择">
                <el-option v-for="item in floorAreaOptions"
                           :key="item.value"
                           :label="item.label"
                           :value="item.value">
                </el-option>
        ...
        <div class="step2"
             v-show="newlystep2">
          <div class="dc-right">
            <div class="building-title"><i></i>
              楼层信息<span v-if="addData.buildingArea && addData.provideFloorArea === true" style="font-weight: normal;font-size: 12px;">(当前楼宇总可用面积：<span>{{addData.buildingArea}}</span> ㎡)</span>
            </div>
            <el-form>
              <div style="padding-bottom:20px;"
                ...
                <div class="floorwrap">
                  <div class="fw-head">
                    ...
                    <span v-if="addData.provideFloorArea === true">可用面积<i v-if="addData.provideFloorArea === true" class="required">*</i>
                      <el-tooltip class="item"
                            effect="dark"
                            placement="right-start">
                        <div slot="content">
                  ...
                  <div class="fw-content"
                    ...
                    <span v-if="addData.provideFloorArea === true">
                      <el-input placeholder="请输入内容"
                                style="width:160px;"
                                v-model="item.buildingArea">
                                <template slot="append">㎡</template>
                      </el-input>
                    </span>
                  </div>
  ...
  methods: {
    ...
    newlystepNoe (formName) {
      if (this.addData.assetsType === 'BUILDING') {
        ...
        if (this.addData.assetsType === 'BUILDING' &&
            this.addData.buildingBusiness === 'SELF_USE' &&
            this.addData.buildingArea &&
            this.addData.canRentArea &&
            Number(this.addData.canRentArea) > Number(this.addData.buildingArea)) {
          this.$message({
            message: '可出租面积不可大于可用面积',
            type: 'warning'
          })
          return
        }
        if (this.addData.provideFloorArea !== true && this.addData.provideFloorArea !== false) {
          this.$message({
            message: '请选择是否可提供所有楼层面积',
            type: 'warning'
          })
          return
        }
    ...
    newlystepNoe2 () {
      ...
      if (areat && this.addData.provideFloorArea === true) {
        this.$message({
          message: '可用面积不可为空',
          type: 'warning'
        })
    ...
    async addStaff (formName) {
      if (this.buildDetailsShow) {
        ...
        if (!postData.canRentArea) {
          postData.canRentArea = 0
        }
