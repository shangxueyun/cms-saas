        ...
        <el-table :data="buildingData.buildingListDTOS"
          ...
          <el-table-column label="楼宇类型"
            ...
            <template slot-scope="scope">
              <span style="margin-left: 10px">{{filterAssetsType(scope.row.buildingBusiness)}}</span>
            </template>
          </el-table-column>
          ...
          <el-table-column label="已租/已售面积㎡"
                           align="center">
            <template slot-scope="scope">
              <span style="margin-left: 10px">{{ formatManagerArea(scope.row) }}</span>
            </template>
          </el-table-column>
          <el-table-column label="剩余可租/可售面积㎡"
                           prop="yzArea"
                           align="center">
            <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    formatManagerArea (row) {
      ...
      if (type === 'BUILDING' && row.rentSellArea) {
        let managerArea = Number(row.rentSellArea)
    ...
    formatYzArea (row) {
      let temp = null
      let type = row.assetsType
      if (type === 'BUILDING' && row.wzArea) {
        let wzArea = Number(row.wzArea)
        temp = wzArea.toFixed(2)
      }
      return temp || '-'
    },
    formatAreaRate (row) {
