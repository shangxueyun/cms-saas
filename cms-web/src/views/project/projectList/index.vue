      ...
      <div class="projectList_content">
        ...
        <div class="plc-list"
          ...
          <div class="list-bar">
            ...
            <p class="p1"><span>楼宇总数：{{ item.buildingAmount ? item.buildingAmount : '-' }}</span><span>总面积：{{ item.area ? `${item.area} m²` : '-' }}</span><span>一手出租率：{{ item.rentRate ? `${item.rentRate}%` : '-' }}</span></p>
            <p class="p2"><span>入驻企业数：{{ item.enterprisesAmount ? item.enterprisesAmount : '-' }}</span><!-- <span>税收落地企业数：{{ item.ssldenterprisesAmount ? item.ssldenterprisesAmount : '-' }}</span> -->
            </p>
          </div>
