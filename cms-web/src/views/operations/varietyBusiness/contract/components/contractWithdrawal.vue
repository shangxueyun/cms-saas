        ...
        <el-form :model="addData"
          ...
          <div class="step1"
            ...
            <div class="dc-left float-left"
                 style="display: inline-block;min-height: 319px">
              <div class="dr-head"
              ...
              <div class="basic-select">
                <div class="clearfix selectInput"
                  ...
                  <span v-if="detailData.projectManagement"
                        style="margin-left:10px;">{{detailData.projectManagement.referred}}</span>
                </div>
                <div class="bd-number"
                     style="padding: 0">
                  <span>编号</span>
            ...
            <div class="dc-right float-right"
              ...
              <el-form-item :label-width="formLabelWidth"
                ...
                <span v-if="detailData.tenant"
                      style="width: 180px;overflow: hidden;white-space: nowrap;text-overflow: ellipsis;"
                      :title="detailData.tenant.teantName">{{detailData.tenant.teantName}}</span>
              </el-form-item>
...
export default {
  ...
  methods: {
    ...
    getAmount (item, index) {
      this.$nextTick(() => {
        let amountVal = amountFormat(this.addData.otherFeeDTOS[index].amount)
        this.addData.otherFeeDTOS[index].amount = amountVal
      })
    },
    getOtherExpenses () {
      let otherExpenses = 0
    ...
    rentBill () {
      ...
      context.http.get('/cms/api/history-bills/lease', { managementContractId: this.detailData.id, leaseType: this.addData.leaseType, withdrawalDate: times }).then(res => {
    ...
    initiateApproval () {
      this.$refs['addData'].validate((valid) => {
        if (valid) {
          ...
          this.addData.managementContract = contractDTO
          ...
          context.http.post('/cms/api/lease-contracts/management', this.addData).then(res => {
            if (res.status === 201) {
