<template>
  <div id="repairManagement"
    ...
    <span class="page_head_title"
          v-if="!isestate">报修信息</span>
    <span class="page_head_title"
          v-if="isestate">设备设施</span>
    ...
    <div class="menuTab"
      ...
      <span class="active">报修列表<span v-if="repairRdNum.repairForm > 0">{{repairRdNum.repairForm}}</span></span>
      ...
      <span @click="roterTo('deviceRepair')"
            v-if="hasAuthority('03010118')">维保列表</span>
    </div>
    ...
    <div class="search-bar">
      <div class="searbar-left">
        ...
        <el-input v-model="deviceName"
                  ...
                  style="width:150px;margin-right:10px"
                  size="small"></el-input>
        <el-date-picker style="width:250px;margin-right:10px;"
    ...
    <el-dialog class="addbaoxiu"
      ...
      <addRepair1 :detailsdrawershow="addbaoxiuShow"
                  :repairDetails="repairDetails"
                  :projectManagementsList="projectManagementsList"
                  :bulidFloorList="bulidFloorList"
                  :pageType="pageType"
...
export default {
  ...
  methods: {
    ...
    addRecordsSuc () {
      ...
      // this.$refs.repairDetails.getRepair_records()
    },
    addweibaoOpen (repairDetailsData) { // 新增维保
      this.repairDetailsData = repairDetailsData
      this.drawerTitle1 = '新增维保'
    ...
    async handleEdit (index, row) {
      ...
      context.http.get(`/cms/api/repair-forms/${row.id}`, { page: (this.currentPage - 1), size: this.pgSize }).then(res => {
