<template>
  <div class="contracts-info billDetailDrawer">
    <drawer :show="detailsdrawershow"
              ...
              <el-form :model="detailsData"
                ...
                <div class="step1">
                  <!-- 
                    ...
                    tenantName 客户全称
                    contractNo 合同编号
                    areaDescription 租赁面积
                   -->
                  <div class="dc-left float-left"
                       style="text-align: left;min-height: 430px">
                    <div style="margin-bottom:10px;"
                         class="dr-head"><i class="titBorder"></i>客户信息</div>
                    <!-- <div class="basic-select">
                      <div class="clearfix selectInput"
                           style="border: none;margin-bottom: 12px;display: flex;justify-content: flex-start;"
                      ...
                      <div class="bd-number" v-if="detailsData.rooms">{{detailsData.rooms[0].building.buildingName}}</div>
                      <div class="bd-number"
                           v-if="detailsData.rooms">
                        <span>楼层</span>
                        <span>房源号</span>
                        <span v-if="detailsData.areaType === 'USING'">使用面积</span>
                        <span v-else>建筑面积</span>
                      </div>
                      <div class="layer">
                        <div v-for="(item, index) in detailsData.rooms"
                          ...
                          <span v-if="detailsData.areaType === 'USING'">{{item.useArea}}m²</span>
                    ...
                    <div class="tenant-info">
                      <div>
                        <p>管理企业</p>
                        <p>{{ detailsData.projectManagement && detailsData.projectManagement.organizeName }}</p>
                      </div>
                      <div>
                        <p>项目简称</p>
                        <p>{{ detailsData.projectManagement && detailsData.projectManagement.referred }}</p>
                      </div>
                      <div>
                        <p style="width: 56px;">客户名称</p>
                        <p style="max-width: 200px;">{{detailsData.tenantName}}</p>
                      </div>
                      <div>
                        <p>合同编号</p>
                      ...
                      <div>
                        <p style="width: 56px;">租赁面积</p>
                        <p style="max-width: 200px;">{{detailsData.areaDescription}}</p>
                      </div>
                    </div>
                  </div>
                  <div class="dc-right float-right formItemWidth"
                       style="min-height: 430px;position:relative">
                    <div style="margin-bottom:10px;"
                         class="dr-head"><i class="titBorder"></i><span style="font-size: 14px;color:#333333">账单信息</span>
                      <span v-if="hasAuthority('02030307') && isEdit && detailsData.reductionAmount <= 0 && detailsData.billStatus === 'NO'"
                            @click="onEdit"
                            class="editBtn1"><i class='iconfont icon-edit2 icon-edit1'></i>编辑</span>
                    </div>
                    ...
                    <div v-if="!isEdit"
                      ...
                      <el-form-item :label-width="formLabelWidth"
                                    prop="startDate"
                                    style="margin-bottom: 20px;width: 108px">
                        <div class="cttitle">计费周期<i style="color:red">*</i></div>
                    ...
                    <el-form-item v-if="detailsData.rentalMethod === 'GUARANTEEPOINT'"
                      ...
                      <span>{{detailsData.bdAmount | toLocaleString}}</span>
                    </el-form-item>
                    <el-form-item :label-width="formLabelWidth"
                                  prop="amount">
                    ...
                    <el-form-item :label-width="formLabelWidth">
                      <div class="cttitle">账单应收是否包含滞纳金</div>
                      <span v-if="isEdit">{{detailsData.onContainLafeFee ? '是' : '否'}}</span>
                      <el-select v-model="detailsData.onContainLafeFee"
                                 placeholder="请选择"
                                 style="width: 225px"
                                 v-else>
                        <el-option :key="true"
                    ...
                    <el-form-item :label-width="formLabelWidth">
                      ...
                      <span v-if="isEdit">{{detailsData.manualRemake}}</span>
                      <el-input v-else
                                v-model="detailsData.manualRemake"
                                maxlength="70"
                                type="text">
                      </el-input>
                    </el-form-item>
                    <div v-if="!isEdit"
                         style="bottom: 10px"
                         class="savebtns">
                      <div @click="billOnHide">取消</div>
                      <div @click="billSave('addData', 1)">保存</div>
                    </div>
                  </div>
                  ...
                  <div class="dc-right float-right tenantinfo reduction"
                    ...
                    <div class="dr-head clearfix"
                      ...
                      <i class="titBorder"></i>滞纳金
                      <div v-if="!isEditLateFeeRate"
                           class="savebtns"
                           style="bottom: 6px">
                        <div @click="lateFeeOnHide">取消</div>
                        <div @click="billSave('addData', 2)">保存</div>
                      </div>
                    </div>
                    <div class="">
                      <ul class="ul_list">
                        <li>
                          <p>逾期天数</p>
                          <span>{{detailsData.expectedDay ? detailsData.expectedDay : '-'}}</span>
                        </li>
                        <li>
                          <p>滞纳金比例</p>
                          <span v-if="isEditLateFeeRate">{{detailsData2.lateFeeRate ? detailsData2.lateFeeRate : '-'}}</span>
                          <el-input v-else
                                    title=""
                                    step=0.00001
                                    v-model="detailsData2.lateFeeRate"
                                    type="number"></el-input>
                          <span v-if="detailsData2.lateFeeRate">%/天</span>
                          <span v-if="hasAuthority('02030307') && isEditLateFeeRate"
                                @click="onEditLateFeeRate"
                                style="cursor:pointer;color: #DFDFDF"
                                class="iconfont icon-cms_xiugai-"></span>
                        </li>
                        <li>
                          <p>滞纳金/元</p>
                          <span>{{detailsData.lateFee | toLocaleString}}</span>
                        </li>
                        <li>
                          <p>已收滞纳金/元</p>
                          <span>{{detailsData.sjLateFee | toLocaleString}}</span>
                        </li>
                        <li>
                          <p>待收滞纳金/元</p>
                          <span>{{detailsData.qsLateFee | toLocaleString}}</span>
                        </li>
                      </ul>
                    </div>
                    ...
                    <div v-if="detailsData.tradeFlowDTOS && detailsData.tradeFlowDTOS.length > 0">
                      <ul class="body_ul"
                          v-for="(item, index) in detailsData.tradeFlowDTOS"
                          :key="index">
                        <li>
                          <span>{{item.startDate}} ~ {{item.endDate}}</span>
                        </li>
                        <li>
                          <span>{{item.yqAmount | toLocaleString}}</span>
                        </li>
                        <li>
                          <span>{{item.expectedDay}}</span>
                        </li>
                        <li>
                          <span>{{item.lateFee | toLocaleString}}</span>
                        </li>
                      </ul>
                    </div>
                    <div v-else
                  ...
                  <div class="dc-right float-right tenantinfo reduction"
                       style="width: 100%">
                    <div class="dr-head clearfix"
                         style="margin-bottom: 0"><i class="titBorder"></i>账单调整
                      <div class="btn-wrap">
                        <span v-if="detailsData.billStatus !== 'OK' && detailsData.billStatus !== 'STAY_SETTLE' && hasAuthority('02030310') && detailsData.useReductionAmount > 0"
                              class=""
                    ...
                    <div>
                      <ul class="ul_list">
                        ...
                        <li>
                          <p>减免总金额/元</p>
                          <span>{{detailsData.reductionAmount | toLocaleString}}</span>
                        </li>
                        <li>
                          <p>应收金额/元</p>
                          <span>{{detailsData.ysAmount | toLocaleString}}</span>
                        </li>
                        <li>
                          <p>已收金额/元</p>
                          <span>{{detailsData.writeOffAmount | toLocaleString}}</span>
                        </li>
                        <li>
                          <p>待收金额/元</p>
                          <span>{{detailsData.arrearsAmount | toLocaleString}}</span>
                        </li>
                      </ul>
                    </div>
                    <ul class="head_ul">
                    ...
                    <div v-if="detailsData.reductions && detailsData.reductions.length > 0">
                      <ul class="body_ul"
                          v-for="(item, index) in detailsData.reductions"
                          :key="index">
                        <li>{{item.amount | toLocaleString}}</li>
                        <li>{{item.createTime ? item.createTime.substring(0, 10) : ''}}</li>
                        <li>{{item.createUserName}}</li>
                        <li class="viewFile">
                          <div v-for="(item2, idx2) in item.annexes"
                               :key="idx2">
                            ...
                            <ailPreview :ailObj="item2"
                                        :encrypt="true"
                                        @setFile="setFile" />
                            <!-- <span @click="downloadFile2(item2.annexName, item2.annexURL)"
                        ...
                        <li class="revoke">
                          <span @click="revoke(item.id)"
                                v-if="detailsData.onConfirmOK && hasAuthority('02030310')">撤销减免</span>
                        </li>
                      </ul>
                    </div>
                    <div v-else
                  ...
                  <div class="dc-right float-right tenantinfo"
                       style="width: 100%">
                    <div class="dr-head clearfix"
                         style="margin-bottom: 0"><i class="titBorder"></i>匹配流水
                      <div class="btn-wrap"
                      ...
                      detailsData.billStatus !== 'OK' && detailsData.rentalMethod !=='POINTS'">
                        <span style="margin-right:5px!important"
                              @click="newPipeline()"
                              v-if="hasAuthority('02030308') && detailsData.billStatus !== 'STAY_SETTLE'">新增流水</span>
                        <span class=""
                              @click="matchflow()"
                              v-if="hasAuthority('02030309') && detailsData.billStatus !== 'STAY_SETTLE'">匹配流水</span>
                      </div>
                    </div>
                    <ul class="head_ul">
                      <li>流水金额/元</li>
                    ...
                    <div v-if="detailsData.billAndFlows && detailsData.billAndFlows.length > 0">
                      <ul class="billUl body_ul"
                          v-for="(item, index) in detailsData.billAndFlows"
                          :key="index">
                        <li>{{item.billFlowAmount | toLocaleString}}</li>
                        <li>{{item.amount | toLocaleString}}</li>
                        <li :title="item.flowCode">
                        ...
                        <li class="bullLi">
                          <div @click="flowDetail(item.billFlowId)"
                               v-if="hasAuthority('02030315')">查看</div>
                          <div v-if="hasAuthority('02030311') && detailsData.onConfirmOK"
                  ...
                  <div class="dc-right float-right tenantinfo reduction"
                       style="width: 100%">
                    <div class="dr-head clearfix"
                         style="margin-bottom: 0"><i class="titBorder"></i>通知单发送记录
                    </div>
                    <ul class="head_ul">
                      <li style="width:25%">通知单名称</li>
                      <li style="width:25%">通知单生成时间</li>
                      <li style="width:25%">操作人</li>
                      <li style="width:25%">操作</li>
                    </ul>
                    <div v-if="billNoticeRecordData && billNoticeRecordData.length > 0">
                      <ul class="body_ul"
                          v-for="(item,index) in billNoticeRecordData"
                          :key="index">
                        <li style="width:25%">{{item.noticeName}}</li>
                        <li style="width:25%">{{item.userDate}}</li>
                        <li style="width:25%">{{item.userName}}</li>
                        <li style="width:25%">
                          <span class="cheackBtn"
                                @click="cheackBillNotice(item.noticeUrl)">查看</span>
                        </li>
                      </ul>
                    </div>
                    <div class="noData"
                  ...
                  <div v-if="total > 0"
                       class="ctc-foot"
                       style="justify-content: flex-end;padding: 20px 0 30px 20px;box-sizing:border-box;flex-direction:inherit">
      ...
      <div slot="footer"
           class="dialog-footer"
           style="margin:0;margin-top: -6px;display: flex; height:56px; align-items:center;background:#fff;padding:0 20px;justify-content:flex-end">
        <!-- <el-button v-if="hasAuthority('02030312')"
                   type="primary"
                   class="close-button delete-bill"
                   @click="deleBill">删除账单</el-button> -->
        <!-- <span>
        ...
        <div class="turnoverBtn1"
             v-if="detailsData.rentalMethod !== 'RENT' && detailsData.feeTypeName === '租金' && detailsData.turnoverStatus !== 'OK'">
          <span class=""
                @click="showAmountCalculation()">输入营业额</span>
        </div>
        <div class="settle-btn"
             v-if="detailsData.billStatus === 'STAY_SETTLE' && hasAuthority('02030322')"
             @click="showConfirmSettle">确认结清</div>
      </div>
      <!-- <div slot="footer"
            class="dialog-footer">
    ...
    <el-dialog v-if="centerDialogVisible"
      ...
      <span class="warnInfo">确定删除账单？</span>
      <span slot="footer"
            class="dialog-footer">
        <el-button @click="centerDialogVisible = false">取 消</el-button>
        <el-button type="primary"
                   @click="deleBillTrue">确 定</el-button>
      </span>
    </el-dialog>
    <!-- 解除匹配确认弹窗 -->
    ...
    <!-- 减免撤销确认弹窗 -->
    <el-dialog v-if="reductionDialogVisible"
               :visible.sync="reductionDialogVisible"
               class="lockDialogClose"
               append-to-body
               width="400px">
      <div class="lockDialogContent">
        ...
        <div>
          <div class="div1">减免撤销</div>
          <div class="div2">点击“确定”，该条减免记录将消失！</div>
        </div>
      </div>
      <span slot="footer"
            class="dialog-footer">
        <span class="cancel"
              @click="reductionDialogVisible = false">取消</span>
        <span class="determine"
              @click="confirmReduction">确定</span>
      </span>
    </el-dialog>
    <!-- 新增流水 -->
        ...
        <div class="part part2">
          ...
          <div class="dl-subtitle">
            <div class="enclosure"
                 style="width:100%">
              <div class="enclosure-head"
                   style="width: 140px;position: relative;display: flex;">附件
      ...
      <div class="dialog_content">
        <div style="margin-bottom:10px;"
          ...
          <ul class="ul_list">
            ...
            <li v-if="detailsData1.specificTenant && detailsData1.projectManagement">
              <p>项目简称</p>
              <span v-if="detailsData1.projectManagement">{{detailsData1.projectManagement.referred}}</span>
            </li>
            <li v-if="detailsData1.tenant">
              <p>客户名称</p>
              <span :title="detailsData1.tenant.teantName">{{detailsData1.tenant.teantName}}</span>
            </li>
            <li>
              <p>流水金额/元</p>
        ...
        <div class="flow_list">
          ...
          <el-table key="selectTable1"
            ...
            <el-table-column align="center"
                             label="操作">
              <template slot-scope="scope"><span v-if="!scope.row.isEdit"
                      class="delete_btn"
      ...
      <div class="dialog_content">
        <div style="margin-bottom:10px;"
          ...
          <ul class="ul_list">
            ...
            <li>
              <p>客户名称</p>
              <span :title="detailsData.referred">{{detailsData.tenantName}}</span>
            </li>
            <li>
              <p>账单金额/元</p>
            ...
            <li>
              <p>应收金额/元</p>
              <span>{{detailsData.ysAmount}}</span>
            </li>
            <li>
              <p>待收滞纳金/元</p>
              <span>{{detailsData.qsLateFee}}</span>
            </li>
            <li>
              <p>待收金额/元</p>
        ...
        <div style="margin-bottom:10px;"
             class="flow_list">
          <div class="dr-head"
               style="margin-bottom: 0"><i class="titBorder"></i>待匹配流水</div>
          <div class="search-bar"
               style="margin:10px 0">
            <div style="display:flex;align-items:center">
        ...
        <div class="flow_list">
          <div style="margin-bottom:8px;"
               class="dr-head"><i class="titBorder"></i>匹配流水</div>
          <el-table key="selectTable"
            ...
            <el-table-column align="center"
              ...
              <template slot-scope="scope"><span v-if="!scope.row.isEdit"
                      class="delete_btn"
                      @click="deleteFlow(scope.row, scope.$index)">删除</span></template>
            </el-table-column>
          </el-table>
        </div>
    ...
    <el-dialog v-if="exemptionDialogVisible"
               ...
               width="400px"
               top="26vh"
               class="exemptionDialog addjianmian">
      <el-form :model="reduction"
               :rules="reductionRules"
               ref="reductionform">
        <div style="width:100%"
             class="dl-subtitle">减免金额<span class="required">*</span>
          <el-form-item prop="amount">
            <el-input type="number"
                      title=""
                      step=0.001
                      v-model="reduction.amount">
            </el-input>
          </el-form-item>
        </div>
        <div style="width:100%"
          ...
          <el-form-item>
            <div style="padding-top:10px;">
              附件
              <!-- <span class="upload"
                    v-if="reduction.annexes.length < 5">
              ...
              <aliUpload class="aliUpload aliUpload_small"
                         style="display: inline-block;width:82px;height:24px"
                         upLoadText="上传附件"
                         :encrypt="true"
              ...
              <div class="enclosure-list">
                <div class="el-item"
                     v-for="(item, index) in reduction.annexes"
                     :key="index">
      ...
      <div slot="footer"
           class="dialog-footer">
        <el-button style="background: rgb(255, 255, 255) !important; border: 0px;height: 32px!important!important;"
                   @click="exemptionDialogVisible = false"
                   class="close-button">取消</el-button>
        <el-button style="margin-left: 0px; width: 90px !important;height: 32px!important;line-height: 0!important"
                   type="primary"
    ...
    <!-- 输入操作密码 -->
    <el-dialog v-if="passwordDialogVisible"
               title="输入操作密码"
               :visible.sync="passwordDialogVisible"
               append-to-body
               width="400px"
               top="26vh"
               class="exemptionDialog addjianmian input-password">
      <el-form>
        <div style="width:100%;display: flex;margin: 10px 0 15px 0"
             class="dl-subtitle">
          <span>密码</span>
          <el-form-item>
            <el-input v-model="password"
              ...
              <template slot="append">
                <i class="el-icon-pwd icon"
                   ...
                   :class="{'el-icon-confirm': pwdType === 'text'}"></i>
              </template>
            </el-input>
          </el-form-item>
        </div>
...
export default {
    ...
    detailsData: {
      handler: function () {
        ...
        if (this.detailsData.id) {
          this.currentPage = 1
          this.getBillNoticeRecord(this.detailsData.id) // 请求账单发送记录dto
        }
        ...
        if (this.exemptionType === 'edit') {
          ...
          this.reduction = { annexes: [] }
        }
        if (this.detailsData.reductions && this.detailsData.reductions.length > 0) {
          this.detailsData.reductions = this.detailsData.reductions.filter((item, index) => {
            return item.approvalStatus === 'OK'
          })
        }
        this.detailsData2 = JSON.parse(JSON.stringify(this.detailsData))
        this.isEdit = true
        this.isEditLateFeeRate = true
      },
      immediate: true
    }
  },
  ...
  methods: {
    ...
    getBillNoticeRecord (billId) {
      context.http.get('/cms/api/billNoticeRecord/', { billId: billId, page: (this.currentPage - 1), size: this.pgSize }).then(res => {
    ...
    current_change (currentPage) {
      ...
      this.getBillNoticeRecord(this.detailsData.id)
    },
    hasAuthority (code) {
    ...
    onEditLateFeeRate () {
      ...
      this.isEditLateFeeRate = false
    },
    flowDetail (billFlowId) {
      if (this.$route.path === '/home/billList' || this.$route.path === '/home/flow' || this.$route.path === '/home/flowBank') {
        this.$emit('showFlow', billFlowId)
      } else {
        this.$router.push({ path: '/home/flowBank', query: { billFlowId: billFlowId } })
      }
    },
    ...
    billSave (name, type) {
      ...
      if (type === 1) {
        ...
        detailsData = this.detailsData2
      }
      this.$refs[name].validate((valid) => {
        if (valid) {
          let _this = this
          context.http.put('/cms/api/bills', detailsData).then(res => {
            if (res) {
              _this.$message.success('保存成功')
              _this.$emit('billsSubSuccess', this.detailsData.businessId, 'showFalse')
              if (type === 1) {
                this.billOnHide()
              } else {
                this.lateFeeOnHide()
    ...
    exemptionVisible (type) {
      ...
      if (type === 'add') {
        this.reduction = { annexes: [], amount: this.detailsData.useReductionAmount }
      }
    },
    ...
    amountCalculation () {
      ...
      if (this.BillAmount > this.detailsData.bdAmount) {
        this.detailsData.amount = this.BillAmount
      } else {
        this.detailsData.amount = this.detailsData.bdAmount
      }
    ...
    onHide (index, state) {
      if (state === 'close') {
        this.$emit('drawerClose', this.detailsData.businessId)
      } else {
    ...
    imageChange () {
      ...
      context.http.post('/ywm/api/image-multipart', form, { headers: { 'Content-Type': 'multipart/form-data' } }).then(res => {
        if (res.status === 201) {
          ...
          this.reduction.annexes.push({ annexName: name, annexURL: res.data })
        }
    ...
    downloadFile2 (name, url) {
      axios({ method: 'GET', url: '/ywm/api/annex/download', params: { annexURL: url, annexName: name }, responseType: 'blob' }).then(res => {
        let url = window.URL.createObjectURL(new Blob([res.data]))
        let link = document.createElement('a')
        link.style.display = 'none'
        link.href = url
    ...
    deleFile (item, index) {
      this.reduction.annexes.splice(index, 1)
    },
    billDetails () {
    },
    selectName () { },
    ...
    settle () {
      context.http.post('/cms/api/reductions', postData).then(res => {
        if (res.status === 200) {
          this.$message.success('账单已结清')
          this.$emit('handleEdit', 0, this.detailsData)
        }
      })
    },
    async showConfirmSettle () {
      let result = await context.http.get('uaa/api/account/user')
      if (!result.data.operatePassword) { // 是否已设置密码
        this.$message({
          message: '请先设置操作密码，再“确认结清”',
          type: 'error',
          duration: 1500
        })
        return
    ...
    confirmSettle () {
      context.http.post('/cms/api/bill/billFlow/confirm', { billId: this.detailsData.id, password: this.password }).then(res => {
    ...
    confirmReduction () {
      let id = this.reductioId
      context.http.delete('/cms/api/reductions/' + id).then(res => {
    ...
    matchflow () {
      ...
      context.http.get('/cms/api/billFlows/nomate?sort=recordDate,asc&sort=id,asc', { billId: this.detailsData.id, billType: this.detailsData.billType, tenantId: this.detailsData.tenandId, startDate: '', endDate: '' }).then(res => {
        if (res) {
          res.data.forEach(item => {
            item.drawee = item.paymentName
            item.accountDate = item.recordDate
            item.payAmount = this.setPayAmount(item)
          })
    ...
    querySearch () {
      ...
      context.http.get('/cms/api/billFlows/nomate?sort=recordDate,asc&sort=id,asc', { billId: this.detailsData.id, billType: this.detailsData.billType, tenantId: this.detailsData.tenandId, flowCode: this.flowSearchData.flowCode, startDate: this.flowSearchData.startDate, endDate: this.flowSearchData.endDate }).then(res => {
        if (res) {
          res.data.forEach(item => {
            item.drawee = item.paymentName
            item.accountDate = item.recordDate
    ...
    setPayAmount (item) {
      if (this.detailsData.amount - this.detailsData.reductionAmount > this.detailsData.writeOffAmount) {
        if (this.detailsData.amount - this.detailsData.reductionAmount - this.detailsData.writeOffAmount > item.wpAmount) {
          item.payAmount = item.wpAmount
        } else {
          item.payAmount = this.detailsData.amount - this.detailsData.reductionAmount - this.detailsData.writeOffAmount
        }
      } else {
        if (this.detailsData.qsLateFee > item.wpAmount) {
          item.payAmount = item.wpAmount
        } else {
          item.payAmount = this.detailsData.qsLateFee
        }
      }
      return Number(item.payAmount.toFixed(2))
    },
    ...
    saveFlow () {
      ...
      multipleSelection.forEach((item, index) => {
        arrAmount += Number(item.payAmount)
        ...
        if (Number(item.payAmount) > item.wpAmount) {
          this.$message({
            showClose: true,
      ...
      if (arrAmount > detailsData.arrearsAmount) {
        this.$message({
          showClose: true,
          message: `匹配金额不能大于账单的待收金额`,
          type: 'error'
        })
        return
      }
      // console.log(arr)
      // detailsData.billFlows = arr
    ...
    async newPipeline () {
      ...
      this.addData.organizeId = this.detailsData.projectManagement.organizeId
              ...
              if (matching === 'matching') {
                context.http.get(`cms/api/billFlows/${res.data.id}`).then(res => {
                  ...
                  _this.flowSearchData1 = {
                    ...
                    referred: this.addData.tenant.teantName ? this.addData.tenant.teantName : this.detailsData.tenantName,
                    feeType: '',
                    startDate: '',
                    endDate: ''
                  }
    ...
    async searchOperation (queryString, callback) {
      ...
      let result = await context.http.get('cms/api/tenant/nameLike/rent?name=' + this.addData.tenant.teantName)
  ...
  .billDrawer {
    .drawer {
      ...
      .delete-bill {
        width: 90px;
        height: 32px;
        background: rgba(217, 46, 32, 0.05) !important;
        border: 1px solid rgba(217, 46, 32, 1) !important;
        border-radius: 4px;
        ...
        .main-content {
          ...
          .savebtns {
            ...
            div {
              ...
              &:last-child {
                background: #2a6ef0;
                box-shadow: 0 1px 2px 0 rgba(79, 139, 255, 0.5);
                color: #fff;
              }
            }
          }
