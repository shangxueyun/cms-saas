    ...
    <div class="dl-content clearfix">
      <el-form :model="addData"
        ...
        <div class="step1"
          ...
          <div class="dc-project">
              <div class="dr-head" style="text-align: left">项目选择</div>
              <div class="cttitle" style="margin-left: 16px;">所属项目<i class="required">*</i></div>
              <div class="basic-select" style="height: auto" v-if="houseReadOnly">
              ...
              <div class="basic-select" v-else>
                <div @click="choose('project')"
                  ...
                  <span class="iconfont" :class="{'icon-right': !selProject, 'icon-xiala-': selProject}"></span>
                </div>
                <ul v-if="selProject"
                    class="selectBox">
          ...
          <div class="dc-right float-right tenantinfo">
            ...
            <el-form-item :label-width="formLabelWidth">
              ...
              <el-input v-model="addData.tenant.industry" disabled="disabled"></el-input>
            </el-form-item>
            <el-form-item :label-width="formLabelWidth">
              <div class="cttitle">品牌</div>
              <el-input v-model="addData.tenant.referred" disabled="disabled"></el-input>
            </el-form-item>
            <el-form-item :label-width="formLabelWidth" style="vertical-align: top;">
              <div class="cttitle">法人</div>
              <el-input v-model="addData.tenant.operName" disabled="disabled"></el-input>
            </el-form-item>
          </div>
          <div class="dc-right float-right" ref="aaaaaa">
            <div class="dr-head">房源信息</div>
            <div class="berth-infor" v-show="!berthSelected" style="max-height: 318px;overflow-y: scroll;">
              <div class="berth">
                <span>选择房源</span>
                <span @click="berthSelected = true">完成</span>
              </div>
              <el-collapse accordion style="width: 96%;margin: 10px auto 0" @change="handleChange" v-model="activeName">
                <el-collapse-item v-for="(item, index) in building.projectRoomsSecondDTOList" :key="item.buildingId" :name="index" style="position: relative;">
                  <template slot="title">
                    <img class="floor-img" v-if="item.buildingImage" :src="item.buildingImage" /><span>{{item.buildingName}}</span>
                    <span class="currentlySelected" v-if="item.selectLength && item.selectLength > 0">{{item.selectLength}}</span>
                  </template>
                  ...
                  <el-table
                    ...
                    <el-table-column
                      ...
                      <template slot-scope="scope" v-if="scope.row.useArea">{{ scope.row.useArea }}㎡</template>
                    </el-table-column>
                  </el-table>
                </el-collapse-item>
              </el-collapse>
            </div>
            <div class="berth-selected" v-show="berthSelected">
              <div class="berth" style="margin-bottom: 10px">
                <span>已选中房源</span>
                <span @click="berthSelected = false" v-if="!houseReadOnly">添加房源</span>
              </div>
              <el-table
            ...
            <div class="berth-area" style="margin-top: 14px">
              <form>
                <el-form-item :label-width="formLabelWidth"
      ...
      <el-form>
        <div class="step2 dl-content"
          ...
          <div class="typeTab">
            <span v-for="(item,index) in options4" :key="item.id"  :class="{active:berthPlan === item.value}" @click.stop="changeTab(item,index)">{{item.label}}
              <span class="iconfont icon-cha" v-if="item.value === 'PropertyFeeClause'" @click.stop="delTab(index)" style="font-size: 14px;margin-left: 6px;"></span>
            </span>
            <span class="add-clause" v-if="options4.length === 1" @click="addTal()"> + 物业费条款</span>
          </div>
          <el-form ref="LeaseClauseForm"
                   :model="LeaseClauseData"
                   :rules="LeaseClauseRules"
                   label-width="80px">
            <div class="dc-right float-right" v-show="isrentClause">
              <div class="dr-head">
                计算方式
              </div>
              <div>
                ...
                <el-form-item :label-width="formLabelWidth"
                              ...
                              >
                  <div class="cttitle">计租面积<i class="required">*</i></div>
                  <el-input 
                            disabled="disabled"
                ...
                <el-form-item :label-width="formLabelWidth"
                              style="margin-bottom: 20px;"
                              prop="carryModeRent">
                  <div class="cttitle">计租进位方式<i class="required">*</i></div>
            ...
            <div class="dc-right float-right" v-show="isrentClause">
              ...
              <div class="selected-housing"
                ...
                <el-form-item :label-width="formLabelWidth"
                              style="margin-bottom: 20px;margin-right: 5px;width: 28%!important;"
                              prop="deposit">
                  <div class="cttitle">租金保证金金额<i class="required">*</i></div>
                  <el-input type="number"
                            v-model="LeaseClauseData.deposit"
                            placeholder="请输入内容"></el-input>
                </el-form-item>元
                <el-form-item prop="price" v-if="addData.rentalMethod === 'RENT'">
                  ...
                  <el-select v-model="LeaseClauseData.priceUnit"
                             style="width: 35%;"
                             placeholder="请选择" @change="priceUnitChange">
                    <el-option label="元/㎡天"
                ...
                <el-form-item prop="price" v-if="addData.rentalMethod === 'GUARANTEEPOINT'">
                ...
                <el-form-item :label-width="formLabelWidth"
                              prop="yearDay"
                              v-if="LeaseClauseData.billingType === 'MONTH' && addData.rentalMethod !== 'POINTS'"
                              style="margin-bottom: 20px;">
                ...
                <el-form-item :label-width="formLabelWidth"  style="margin-bottom: 20px;" v-if="LeaseClauseData.month===3||LeaseClauseData.month==='3'">
                  <div class="cttitle">账单划分方式</div>
                  <el-select v-model="LeaseClauseData.billDivision" placeholder="请选择">
                    <el-option label="无做特殊处理" value="NO"></el-option>
                    <el-option label="首期三个月，第三笔开始按季度划分" value="QUARTER"></el-option>
                  </el-select>
                </el-form-item>                             
                <el-form-item v-if="addData.rentalMethod === 'RENT'"
                              :label-width="formLabelWidth"
                ...
                <el-form-item v-if="addData.rentalMethod !== 'RENT'"
                              ...
                              style="margin-bottom: 20px"
                              prop="dayNumber">
                  <div class="cttitle">次月第几个<i class="required">*</i></div>
                  <el-input v-model="LeaseClauseData.dayNumber"  style="width: 83%!important"
                            type="number"
                            min="1"
                            max="31"
          ...
          <div class="dc-right float-right"
            ...
            <div class="selected-housing"
              ...
              <el-form-item :label-width="formLabelWidth" v-if="addData.rentalMethod === 'RENT'"
              ...
              <el-form-item v-if="addData.rentalMethod !== 'RENT'"
                            ...
                            prop="point">
                <div class="cttitle">租金抽成比例(按营业额抽成)<i class="required">*</i></div>
                <el-input v-model="item.point" style="width: 83%!important"
                          type="number"
                          placeholder="请输入内容"></el-input>
                          <span>%/月</span>
              </el-form-item>
              <el-form-item v-if="addData.rentalMethod === 'GUARANTEEPOINT'">
                <div class="cttitle">保底价<i class="required">*</i></div>
                <el-input v-model="item.number"  style="width: 59%;margin-right: 10px" placeholder="请输入内容"></el-input>
                <el-select v-model="item.priceUnit" style="width: 35%;" placeholder="请选择">
          ...
          <el-form ref="propertyClauseForm" v-show="!isrentClause"
            ...
            <div class="dc-right float-right">
              ...
              <div class="selected-housing"
                ...
                <el-form-item :label-width="formLabelWidth"
                              style="margin-bottom: 20px;margin-right: 5px;width: 28%!important;"
                              prop="deposit">
                  <div class="cttitle">物业费保证金金额<i class="required">*</i></div>
                ...
                <el-form-item :label-width="formLabelWidth"
                              prop="yearDay"
                              v-if="propertyClauseData.billingType === 'MONTH'"
                              style="margin-bottom: 20px;">
                  <div class="cttitle">年天数<i class="required">*</i></div>
                ...
                <el-form-item :label-width="formLabelWidth"
                              style="margin-bottom: 20px;width: 25%!important"
                              prop="dayNumber">
                  <div class="cttitle">提前付款时间<i class="required">*</i></div>
          ...
          <div class="dc-right float-right">
			...
			<div class="step3 dl-content" v-show="newlystep3">
				<div class="dc-right float-right" style="width: 100%">
					...
					<div class="other-info" v-if="houseReadOnly">
						<el-input type="textarea" :rows="2" resize="none" v-model="addData.remark">
						</el-input>
					</div>
					<div class="other-info" v-else>
						<el-input type="textarea" :rows="2" resize="none" v-if="addData.remarks[0]" v-model="addData.remarks[0].content">
						</el-input>
					</div>
				</div>
				<div class="dc-right float-right" style="width: 100%">
		...
		<div slot="footer" class="dialog-footer">
			...
			<el-button type="primary" class="close-button" v-if="newlystep3 &&  contractState !== 'renewal' && !approvalAgain "
...
export default {
  ...
  data () {
    ...
    return {
      ...
      carryOptions: [
        {label: '四舍五入制', value: 'ROUNDING', id: 1},
        {label: '无条件进位制度', value: 'UNCONDITIONAL', id: 2}
      ],
      options4: [
  ...
  watch: {
    projectList: {
      handler: function () {
        ...
        console.log(localStorage.STORAGE) // 合同暂存查看
  ...
  methods: {
    ...
    icVerification () {
      ...
      if (this.propertyClauseData.incrementRates.length > 0 && this.increasingProperty && !this.isrentClause) {
      ...
      if (this.propertyClauseData.offerRecords.length > 0 && this.propertyFee && !this.isrentClause) {
      ...
      if (this.propertyClauseData.areaTypeProperty === 'BUILDING' && Number(this.addData.area) === 0 && !this.isrentClause) {
        this.$message.error('计租面积为0')
        result = false
      } else if (this.propertyClauseData.areaTypeProperty === 'USING' && Number(this.addData.useArea) === 0 && !this.isrentClause) {
        this.$message.error('计租面积为0')
        result = false
      }
      return result
          ...
          if (valid) {
            this.$refs['propertyClauseForm'].validate((valid) => {
              if (valid) {
                ...
                this.addData.areaTypeProperty = this.propertyClauseData.areaTypeProperty
                this.addData.carryModeProperty = this.propertyClauseData.carryModeProperty
                context.http.post('/cms/api/bills/business', this.addData).then(res => {
                  this.rentBillDetails = res.data
                })
              } else {
                this.isrentClause = false
