<template>
  <div id="contractTenancy"
    ...
    <div class="dl-head">
      ...
      <span class="step"
            :class="{stepActive: newlystep2}"><span style="font-size: 20px;">2</span>. 减免信息</span>
      <el-button
        v-if="NunOnRent != 2"
                    ...
                    <div class="node-wrap-box"
                      ...
                      <div>
                        <div class="title"
                          ...
                          <span>
                            <i v-if="item.type !== 'START' && item.type !== 'CC' "
                               :class="{
      ...
      <div class="newlystep2"
        ...
        <div class="dl-content clearfix">
          <div class="dc-right float-right"
            ...
            <div class="dr-head"
                 style="margin-bottom: 0">减免申请</div>
            <div class="other-info derateDetails"
              ...
              <div class="derateConTENT">
                ...
                <span>{{contractInfo.timeReduction}}</span>
                <span>{{formatDerateType(contractInfo.reliefType)}}</span>
              </div>
            </div>
          </div>
          <div class="dc-right float-right"
            ...
            <div class="other-info"
                 style="min-height:100px;padding:20px;color:#999999;background-color: #fff;"> <span>{{contractInfo.remake}}</span></div>
          </div>
          <div class="dc-right float-right"
               style="width: 100%">
            <div class="enclosure">
              <div class="enclosure-head">附件</div>
              ...
              <div class="enclosure-list">
                <div class="el-item"
                     style="display: flex"
                     v-if="contractInfo.annexName">
                  <span>{{contractInfo.annexName}}</span>
                  <span style="display: flex">
                    <ailPreview :ailObj="item"
                                :encrypt="true"
                                :picturePreview="true" />
                    <i @click="downloadFile(contractInfo)"
          ...
          <div class="dc-right float-right"
               style="width: 100% ;margin:20px 0 40px 0;border:none;border: 1px solid #eee;"
               v-if="newlystep2 && approvalOperation">
            <div class="dr-head"
                 style="margin-bottom: 0">审批意见<i style="color:red"> * </i></div>
            <div class="textArea"
                 style="background-color:#ffffff;height: 96px">
...
export default {
  ...
  methods: {
    ...
    async surrenderInfo (id) { // 减免审批详情
      let result = await context.http.get(`/cms/api/reductions/${id}`)
    ...
    newlystepNoe () {
      if (this.contractStatus === 'START' && !this.finshed) {
        context.http.get(`/cms/api/reductions/${this.contractInfo.id}`).then(res => {
          this.reduction = res.data
    ...
    reductions (formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          postData.taskId = this.$route.params.row.taskId
          context.http.put('/cms/api/reductions', postData).then(res => {
            if (res) {
              _this.exemptionDialogVisible = false
              _this.$router.push({ name: 'approvalCenter', params: { onRent: this.$route.params.onRent } })
            }
          })
        } else {
