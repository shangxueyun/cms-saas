      ...
      <div class="plc-search search-bar">
        <div>
          ...
          <el-select v-model="approvalType"
                     ...
                     v-if="this.onRent!==2"
                     placeholder="审批类型">
            <el-option v-for="item in approvalTypeOptionsEx"
                       :key="item.id"
                       :label="item.value"
                       :value="item.id">
            </el-option>
          ...
          <el-input :placeholder="this.onRent===1 ? '审批流程名称/审批标题/客户' : '审批流程名称/审批标题/客户/发起人'"
                    v-model="title" 
                    size="small"
                    v-else
...
export default {
  ...
  methods: {
    ...
    setQueryCriteria () {
      let organizeId = ''
      if (localStorage.getItem('newApprovalPmId')) {
        organizeId = localStorage.getItem('newApprovalPmId').split(',')
        organizeId = organizeId.map(item => {
          return Number(item)
        })
      }
      this.organizeId = organizeId
      if (Number(localStorage.getItem('newApprovalApprovalType')) === 0) {
        this.approvalType = 0
      } else {
    ...
    async handleChange1 (organizeId, type) {
      if (this.onRent !== 0) {
        return
      }
      let result = await context.http.get('cms/api/process-defines/processType?flag=true', { organizeId: organizeId[organizeId.length - 1] })
      if (!type) {
        this.approvalTypeOptionsEx = result.data
      } else {
    ...
    async changeNav (inx) {
      ...
      this.approvalType = ''
      if (inx === 0) {
        await this.handleChange1(this.organizeId, 'init')
        this.newApproval = true
        // this.onRent = false
        this.approvalTypeOptionsEx = this.approvalTypeOptions
      } else {
        await this.getApprovalType()
        // this.onRent = null
        this.newApproval = false
        // this.approvalTypeOptionsEx = this.approvalTypeOptions.slice(0)
        // this.approvalTypeOptionsEx.push({
        //   id: 0,
        //   value: '非标审批'
        // })
      }
    ...
    async queryApproval () {
      if (sessionStorage.getItem('appSearchInfo')) {
        ...
        this.organizeId = appSearchInfo.organizeId
    ...
    currentSel () {
      ...
      // localStorage.setItem('newApprovalPmId', this.organizeId[this.organizeId.length - 1])
      localStorage.setItem('newApprovalPmId', this.organizeId)
    ...
    async getProcessList (url, type) { // 请求列表封装
      ...
      let result = await context.http.get(`/cms/api/${url}` + sort, { page: (this.pager.currentPage - 1),
