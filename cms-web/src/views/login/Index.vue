<template>
  <div id="login"  v-bind:style="{backgroundImage: 'url(' + backgroundImg + ')'}">
    <div class="login-wrap" v-if="islogin">
      ...
      <el-form :model="login" :rules="rules" ref="ruleForm">
        <div class="lw-content">
          ...
          <el-checkbox v-model="readed" class="agreement"><span class="agreement-text">我已认真阅读并接受</span><span class="agreement-link" @click.stop.prevent="UserAgreement">《ecams用户服务协议》</span></el-checkbox>
        </div>
      </el-form>
    </div>
    <div class="login-wrap index-change" v-else>
      <div class="forget-head">
        <h2>忘记密码</h2>
      </div>
      <div class="ft-content">
        <changePassword :loginCode="login.code" @setLoginCode="setLoginCode"  ref="changepwd" @goback="goback"/>
        <div class="confirm-btn" @click="confirm('ruleForm')">确定</div>
        <div>
          <span class="back" @click="islogin=true">《 返回登录</span>
        </div>
      </div>
    </div>
  </div>
