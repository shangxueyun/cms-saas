        ...
        <div class="main_content"
          ...
          <el-form :model="draweDetaileData"
            ...
            <div class="contentItem">
              ...
              <el-form-item class="item">
                <span>所属农贸市场名称<i class="required">*</i></span>
                <p v-if="readOnly">{{draweDetaileData.title}}</p>
                <el-input v-model="draweDetaileData.title"
                          placeholder="请输入"
                          v-else
                          size="small"></el-input>
              </el-form-item>
            ...
            <div class="content">
              <div>
                ...
                <el-time-select
                  style="width: 180px"
                  placeholder="结束时间"
                  v-model="draweDetaileData.endTime"
                  size="small"
                  :editable="false"
                  :readonly="readOnly"
                  :clearable="false"
              ...
              <div class="plc-table">
                <el-table :data="tableData"
                  ...
                  <template slot-scope="scope">
                    <span class="psScope" v-if="readOnly">{{scope.row.currentLimitingCount}}</span>
                    <el-input v-model.number="scope.row.currentLimitingCount"
                          type="number"
                          v-else
                          oninput="value=value.replace(/[^0-9]/g,'');if(value.length>9)value=value.slice(0,9)"
                          size="small"></el-input>
                          <!-- @input="numberChange(arguments[0],999999999999)" -->
                  </template>
                </el-table-column>
                </el-table>
              </div>
...
export default {
      ...
      handler (newValue, oldValue) {
        if (newValue) {
          if (!this.draweDetaileData.startTime && !this.draweDetaileData.endTime) {
            this.$set(this.draweDetaileData, 'startTime', '00:00')
            this.$set(this.draweDetaileData, 'endTime', '24:00')
          }
  ...
  methods: {
    ...
    save () {
      // console.log(this.draweDetaileData.tracedName)
      if (!this.draweDetaileData.title) {
        this.$message.warning('请输入所属农贸市场名称')
        return
      }
      context.http.put(`gos/api/visualConfig`, this.draweDetaileData).then(res => {
        if (!this.draweDetaileData.startTime || !this.draweDetaileData.endTime) {
          this.$message.warning('请输入起止时间')
          return
        }
        let temp = this.tableData.some(item => {
          return (item.currentLimitingCount !== 0 && !item.currentLimitingCount)
        })
        if (temp) {
          this.$message.warning('请输入每小时预约上限')
          return
        }
        this.draweDetaileData.buildingConfigDTOS = this.tableData
        // this.draweDetaileData.startTime = this.orderStartTime
        // this.draweDetaileData.endTime = this.orderEndTime
        context.http.put(`gos/api/visualConfig`, this.draweDetaileData).then(res => {
          if (res) {
    ...
    async getBuildings () {
      const res = await context.http.get(`cms/api/project/${this.draweDetaileData.projectManagementId}/buildings?page=0&size=100&sort=createdDate,desc&useStatus=USING`)
      ...
      if (resData.length > 0) {
        resData.forEach(ele => {
          tableData.push({
            ...
            currentLimitingCount: 0
          })
        })
      }
      if (this.draweDetaileData.buildingConfigDTOS.length !== 0 && tableData.length !== 0) {
        this.draweDetaileData.buildingConfigDTOS.forEach(item => {
          tableData.forEach(value => {
            if (value.buildingId === item.buildingId) {
              value.currentLimitingCount = item.currentLimitingCount
            }
          })
        })
        // tableData = this.draweDetaileData.buildingConfigDTOS
      }
