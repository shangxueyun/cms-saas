      ...
      <div class="plc-table">
        <el-table :data="tableData"
          ...
          <!-- <el-table-column prop="name"
                           label="线上菜场名称"
                           align="center" /> -->
          <el-table-column prop="lastDateTime"
...
export default {
  ...
  data () {
    ...
    return {
      ...
      addFlowRules: {
        ...
        // ],
        title: [
          { required: true, message: '请输入所属农贸市场名称', trigger: 'blur' }
        ]
        // splitAccount: [
  ...
  methods: {
    ...
    projectChange (row, o) {
      // row = this.options1.filter(v => {
      //   if (v.id === row) {
      //     return v
      //   }
      // })
      // this.addData.tracedName = row[0].projectName
    },
    addgoods () {
      this.$refs['addData'].validate((valid) => {
        if (valid) {
          ...
          if (this.addData.onLineSell) {
            ...
            params.title = this.addData.title
          }
          context.http.post(`gos/api/visualConfig`, params).then(res => {
            if (res) {
    ...
    lastModifiedDateFormat (row, column) {
      let time = moment(row.lastDateTime).format('YYYY-MM-DD HH:mm:ss')
      return time
    },
    drawerClose () {
      this.query()
