  ...
  <div id="industryCAD"
    ...
    <div v-if="approvalShow"
      ...
      <div>
        ...
        <el-select v-model="seachParams.buildingName"
                   placeholder="层/楼平面图"
                   @change="getFloorList">
        ...
        <el-select v-model="seachParams.floorName"
                   placeholder="楼层">
...
export default {
  ...
  created () {
    if (this.$route.params.detailsData) {
      ...
      this.seachParams.projectManagementId = this.$route.params.projectManagementId
      this.seachParams.buildingName = this.$route.params.buildingName
      this.seachParams.floorName = this.$route.params.floorName
      this.checkFun()
    } else {
      this.getOrganizes()
    }
  ...
  methods: {
    async organizesChange () {
      this.seachParams.projectManagementId = ''
      this.seachParams.buildingName = ''
      this.seachParams.floorName = ''
      this.projectOptions = []
      this.options2 = []
      this.options3 = []
    ...
    async getBuliding () {
      ...
      if (this.options2.length) {
        this.seachParams.buildingName = this.options2[0].id
      } else {
        this.seachParams.buildingName = null
      }
      this.getFloorList()
    },
    async getFloorList () {
      this.seachParams.floorName = ''
      this.options3 = []
      let result = await context.http.get('cms/api/floors/svg', { projectManagementId: this.seachParams.projectManagementId, buildingId: this.seachParams.buildingName })
      ...
      if (this.options3.length) {
        ...
        this.seachParams.floorName = defultFloorName
      } else {
        this.seachParams.floorName = null
      }
    },
    async getOrganizes () { // 管理企业获取接口
      ...
      if (this.$route.params.buildingId) {
        this.seachParams.buildingName = this.$route.params.buildingId
      } else {
        if (this.options2.length) {
          this.seachParams.buildingName = this.options2[0].id
        } else {
      ...
      context.http.get('cms/api/floors/svg', { projectManagementId: this.seachParams.projectManagementId, buildingId: this.seachParams.buildingName }).then(res => {
        this.options3 = res.data
        if (this.options3.length) {
          if (this.$route.params.floorName) {
            this.seachParams.floorName = this.$route.params.floorName
          } else {
            let defultFloorName = ''
            this.options3.forEach((item, index) => {
    ...
    async checkFun () { // 查看对应平面图
      if (this.seachParams.floorName === '' || this.seachParams.projectManagementId === '') {
        this.$message({
          type: 'error',
          message: '请选择项目和楼层'
        })
    ...
    switchover (state) { // 平面图切换
      let params = {
        organizeId: this.seachParams.organizeId,
        projectManagementId: this.seachParams.projectManagementId,
        buildingId: this.seachParams.buildingName,
        floorId: this.seachParams.floorName
      }
