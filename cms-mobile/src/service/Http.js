...
axios.interceptors.request.use(config => {
  if (!hideLoadingUrl.includes(config.url)) {
  ...
  if (qichachaUrl.indexOf(config.url) >= 0) {
    ...
    } else if (config.url.indexOf('/ywm/api/corporate-customers/companyCode') >= 0 || config.url.indexOf('/ywm/api/corporate-customers/account') >= 0) {
      config.headers['Authorization'] = 'Basic d2ViX2FwcDpjaGFuZ2VpdA=='
    } else {
      if (refreshToken) {
...
axios.interceptors.response.use(config => {
  ...
  if (error.response && error.response.status === 401 && error.response.config.url.indexOf('/uaa/oauth/token') > 0 && window.location.hash !== '#/login') {
