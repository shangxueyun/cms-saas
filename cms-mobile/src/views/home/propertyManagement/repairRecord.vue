  ...
  <div class="repairRecord">
    <van-steps direction="vertical"
      ...
      <van-step class="van-step"
                ...
                :key="index">
        <div class="step-head">
          <span class="record-type_sapn">{{formatRepairStatus(item.repairStatus)}}</span>
          <span>{{item.repairDate}}</span>
        </div>
        <p>{{item.recordUser}}</p>
        <p>
          <span>维修记录：</span>
          <span v-html="item.recordString"></span>
        </p>
    ...
    <!-- 写跟进按钮 -->
    <div class="writeBtn"
         v-if="hasAuthority('03060104') && role==='WUYE'"
         @click="writeBtn"></div>
  </div>
</template>
<script>
...
export default {
  name: 'repairRecord',
  props: ['repairId', 'repairDetails', 'role'],
  ...
  methods: {
    ...
    followRecord (type) {
      context.http.get(`cms/api/repair-forms/${this.repairId}/repair-records`).then(res => {
        if (res.status === 200) {
    ...
    writeBtn () {
      sessionStorage.removeItem('addWeixiuData')
      sessionStorage.setItem('roterFrom', 'repairList')
      this.$router.replace({ name: 'addWeixiu', params: { 'repairDetails': this.repairDetails } })
    }
  }
}
</script>
