  ...
  <div class="repairList"
    ...
    <div class="head">
      <i class="back mintui mintui-back"
         @click="onGoBack"></i>
      <div class="headTab">
        <span class="">维修管理</span>
        <!-- <span @click="tabChange"
              v-if="hasAuthority('03010114')">保养列表</span> -->
      </div>
      <div class="addBtn widthrightBtn"
           @click="showActionSheet"
           :style="hasAuthority('03060102') ? '' : 'right: 0px;'">
...
export default {
  ...
  activated () {
    ...
    if (sessionStorage.getItem('repairStatus')) {
      ...
      // sessionStorage.removeItem('repairStatus')
    }
  ...
  methods: {
    ...
    async query (flag) {
      ...
      let param = { projectId: this.currentProject.id, status: this.repairStatus.value, page: this.page, size: this.size, sort: 'repairFormCode,desc' }
    ...
    goDetails (item) {
      if (!this.hasAuthority('03060103')) {
        Toast({
          message: '你的账号没有查看维修详情的权限',
          position: 'top'
        })
        return
      }
