<template>
  <div class="ContractList">
    <div class="tab" id="tab">
      <span @click="changeTab(0)" class="" :class="{'active': active === 0}">房源合同</span>
      <span @click="changeTab(1)" class="" :class="{'active': active === 1}">多经合同</span>
    </div>
    <div class="contract-div" :style="{minHeight: minHeight}"  v-if="active === 0 && contractsList.length > 0">
    ...
    <div class="nodata" :style="{minHeight: minHeight}" v-if="active === 0 && contractsList.length === 0">
      暂无数据
    </div>
    <div class="contract-div" :style="{minHeight: minHeight}" v-if="active === 1 && multipleList.length > 0">
      <ul v-for="(item, index) in multipleList">
        <li>
          <span class="lease-way">
            <span v-for="(item2, index2) in item.managementPoints"
                :key="item2.id">{{item2.pointName}} <i v-if="item.managementPoints.length !== index2 + 1">、</i></span>
          </span>
          <span class="contractstatus" :class="{
            'EXECUTING' : item.contractStatus === 'EXECUTING',
            'MATURITY' : item.contractStatus === 'MATURITY',
    ...
    <div class="nodata" :style="{minHeight: minHeight}" v-if="active === 1 && multipleList.length === 0">
      暂无数据
    </div>
    <div class="loadAll">
      <span>-</span>
