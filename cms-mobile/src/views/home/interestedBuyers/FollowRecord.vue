  ...
  <div class="FollowRecord">
    <van-steps direction="vertical" :active="0" active-icon="none" active-color="#18191A" v-if="timeLineListData.length > 0">
      <van-step class="van-step" v-for="(item, index) in timeLineListDataShowOff" :key="index">
        <div class="step-head">
          <span class="record-type_sapn" :class="{
        ...
        <p>{{item.followName}}</p>
        <p v-if="item.followRecordType==='FOLLOW'">
          <span>跟进记录：</span>
          <span>{{item.followContent}}</span>
        </p>
        <p v-if="item.followRecordType==='FOLLOW' && item.yxRooms!==''"><span>意向房源：</span><span>{{item.yxRooms}}</span></p>
...
export default {
  name: 'FollowRecord',
  props: ['tenantID', 'active', 'tenantDetail'],
  ...
  data () {
    return {
      timeLineListDataAll: [],
      more: false,
      followTypes: ['CONSTRUCTION', 'FINANCE', 'PATROL', 'OTHERS'],
      ctlist: [
      ...
      followRecordTypes: 'CONSTRUCTION,FINANCE,PATROL,OTHERS',
  ...
  methods: {
    ...
    followRecord (type) {
      context.http.get(`cms/api/followRecords/${this.tenantID}`).then(res => {
        if (res.status === 200) {
    ...
    writeBtn () {
      ...
      this.$router.replace({name: 'interestedFollow', query: {'tenantDetail': JSON.stringify(this.tenantDetail)}})
    }
  }
}
</script>
