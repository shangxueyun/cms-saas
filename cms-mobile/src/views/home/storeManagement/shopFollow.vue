...
export default {
  ...
  methods: {
    ...
    addRecipient () {
      ...
      this.$router.replace({ name: 'selectRecipients', params: { selectPerson: '', 'id': this.$route.query.shopId }, query: { 'shopId': this.$route.query.shopId } })
    },
    onConfirm2 (date) {
    ...
    confirmFn () {
      if (this.isSubmit) {
        this.$router.replace({ name: 'storeDetails', query: { 'shopId': this.$route.query.shopId } })
        return
      }
      let _this = this
      if ((!this.followRecordDTOParams.followTime || this.followRecordDTOParams.followTime === '') &&
        ...
        _this.$router.replace({ name: 'storeDetails', params: { 'id': this.$route.query.shopId } })
        return
      }
      MessageBox.confirm('', {
        message: '内容尚未保存，确定退出？',
    ...
    submit () {
      ...
      let tenantIDObj = {
        roomsId: this.$route.query.shopId
      }
      let addData = { ...this.followRecordDTOParams, ...tenantIDObj }
      addData.followTime = addData.followTime.replace(' ', 'T') + '.000Z'
      addData.zcFollowType = 'ROOM'
      context.http.post('cms/api/roomsFollowRecord', addData).then(res => {
        if (res.status === 201) {
          this.isSubmit = true
          // this.$router.back()
          this.$router.replace({ name: 'storeDetails', params: { id: this.$route.query.shopId } })
          Toast({
            message: '新增成功',
            position: 'top'
          })
