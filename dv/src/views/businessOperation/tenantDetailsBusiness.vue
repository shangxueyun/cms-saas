<template>
  <div class="tenantDetailsBusiness">
    <div class="head">
      <div class="home"
           @click="back"><i class="iconfont icon-kfd_shouyeL"></i> 商业经营数据</div>
      <div class="titleTxt">{{referred}}商业经营数据</div>
      ...
      <div class="tenantInfo">
        <div class="info">
          ...
          <div>
            ...
            <div class="item">
              <span>员工数：</span>
              <span v-if="tenantDetails.employeesNumber">{{tenantDetails.employeesNumber}} 人</span>
              <span v-else> - </span>
            </div>
          </div>
        </div>
        <div class="dataEchars">
          ...
          <div class="chartItem chart2">
            ...
            <div class="chartwrap">
              <div class="common-head">
                <i class="iconfont icon-biaoti"></i>营业额
                <span class="underline"
                      style="width: 50%;"></span>
                <span class="timer">
                  <i :class="{'active':dateType==='MONTH'}"
                     @click="dateTypeChange('MONTH')">月</i>
                  |
              ...
              <div id="tenantTurnover"
                   style="width: 100%;height:90%;"></div>
              <div class="dataDiv">
                <span>本{{dateType==='MONTH'?'月':'年'}}营业额</span>
                <span>{{nowAmount}}万元</span>
              </div>
              <div class="dataDiv dataDiv2">
                <span>上{{dateType==='MONTH'?'月':'年'}}营业额</span>
          ...
          <div class="chartItem chart4">
            ...
            <div class="chartwrap chartwrap3">
              <div id="energyamountGas"
                   style="width: 100%;height:90%;"></div>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    async getDetails (tenantId) {
      ...
      // -----------------------------------------------
      this.getTenantRentData(tenantId) // 客户租金
      this.getTenantTurnoverData(tenantId) // 营业额
    ...
    getTenantRentData (tenantId) { // 客户租金
    ...
    tenantRent (dateArray, amount, sjAmount) { // 客户租金
    ...
    getTenantTurnoverData (tenantId) { // 营业额
      service.http.get(`/cms/api/statistics/tenantTurnover/shop`, {
        tenantId: tenantId,
    ...
    tenantTurnover (dataArr, datearr) { // 营业额
      // 基于准备好的dom，初始化echarts实例
      // let _this = this
      let series = {
        type: 'line',
    ...
    getTenantEnergy (tenantId) { // 能耗费用
      service.http.get(`/cms/api/statistics/tenantEnergy/${tenantId}`).then(result => {
        ...
        this.tenantEnergy(electric, water, gas, datearr)
      })
    },
    tenantEnergy (electric, water, gas, datearr) { // 能耗费用
      // 基于准备好的dom，初始化echarts实例
      // let _this = this
      let series = [
        {
          ...
          itemStyle: {
            ...
            normal: {   // 颜色渐变函数 前四个参数分别表示四个位置依次为左、下、右、上
              areaStyle: {
                color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [{
                  offset: 0.3,
                  color: 'rgba(53,117,246,0.3)'
                },
                {
                  offset: 0.5,
                  color: 'rgba(53,117,246,0)'
                },
                {
                  offset: 1,
                  color: 'rgba(53,117,246,0)'
    ...
    energyamountPower (dateArray, dataArray) { // 用量电
      let myChart = echarts.init(document.getElementById('getEnergyamount'))
      let option = {
        tooltip: {
          trigger: 'axis',
          axisPointer: {
