<template>
  <div class="switch-screen">
    <div class="switch-classification" @click="dialogVisible = true;managementsTypes()" v-if="tabIndex === 0">
      {{classification}}
    </div>
    <div class="title">
    </div>
    <div class="navbar">
      <div class="" @click="selectNav(index)" :class="{navSelected: index === tabIndex}" v-for="(item, index) in navigationBar" :key="item.id">
        {{dataTypeFormat(item.dataType)}}
      </div>
    </div>
    <div class="switch-content">
      <div class="content-left">
        <div class="project-item" @click="goToScreen(projecCachetList1)">
          <div class="pi-title">{{projecCachetList1.referred}}</div>
          <div class="pi-img">
          ...
          <div class="swiper-wrapper">
            <div class="swiper-slide" v-for="(item, i) in projecCachetListShow" :key="i">
              <div class="project-item" @click="goToScreen(item)">
                <div class="pi-title">{{item.referred}}</div>
                <div class="pi-img">
    ...
    <el-dialog
      ...
      <div class="category-content">
        <span v-for="(item, index) in category" :key="item.value" @click="clickHandle(item, index)" :class="{'select': selectIndex === index}">
          {{item.value}}
          <i v-if="selectIndex === index" class="selectIcon"></i>
        </span>
      </div>
      <span slot="footer" class="dialog-footer">
        <span class="cancel" @click="cancel">取 消</span>
        <span class="determine" @click="determine">确 定</span>
      </span>
    </el-dialog>
  </div>
</template>
        ...
        if (res.status === 200) {
          ...
          reslist.forEach(ele => {
            if (ele.mallId) {
              ...
              pjGroup = {referred: companyName, previewImage: _this.groupImg, id: '', hasLimit: true}
            }
          })
